...
class UBTeacherTrainer(DefaultTrainer):
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            for key in record_dict.keys():
                if key[:4] == "loss":
                    if key == "loss_rpn_loc_pseudo" or key == "loss_box_reg_pseudo":
                        ...
                        loss_dict[key] = record_dict[key] * 1
                        old_sup_loss_dict[key] = record_dict[key] * 1
            ...
            for key in record_dict.keys():
                if key[:4] == "loss":
                    if key != "loss_rpn_loc_pseudo" and key != "loss_box_reg_pseudo" and key[-6:] != "pseudo": # supervised loss
                      new_sup_loss_dict[key] = record_dict[key] * 1
            omega = sum(old_sup_loss_dict.values()) - sum(new_sup_loss_dict.values())

            if omega < 0:
              keep_rate = 1.00
            else:
              r = sum(new_sup_loss_dict.values()) / sum(old_sup_loss_dict.values())
              keep_rate = np.sqrt(1-(r-1)^2)
